...
const ILoveYouPage = () => {
  ...
  const [floatingWords, setFloatingWords] = useState<Array<{id: number, word: string, x: number, delay: number, duration: number, color: string}>>([]);
  const [burstHearts, setBurstHearts] = useState<Array<{id: number, x: number, y: number}>>([]);
  ...
  const createHeartBurst = () => {
    const burstArray: {id: number, x: number, y: number}[] = [];
    for (let i = 0; i < 15; i++) {
  ...
  const triggerFireworks = () => {
    ...
    const newParticles: Array<{id: number, x: number, y: number, vx: number, vy: number, color: string}> = [];
